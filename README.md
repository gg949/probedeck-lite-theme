...
- **卡片**：按地区分组（美洲 / 欧洲 / 亚洲 / 大洋洲），每张卡显示状态点、国旗、价格、剩余天数、总流量、
  更新时间、标签 + IPv4/IPv6 徽章（同一行）、后台 Ping 线路延迟，以及 CPU / 内存 / 存储三条进度条和 OS、TCP/UDP、实时网速
...
- 时间范围：1 小时 / 6 小时 / 1 天 / 2 天 / 7 天 / 14 天 / 30 天（选择记忆在 localStorage；访客范围受面板「访客历史范围」限制）
- 七张历史曲线（Chart.js）：CPU、内存、磁盘使用率、进程数、网络速度（入 / 出）、TCP / UDP、磁盘 IO（读 / 写）
- 所有图表悬停查看数据时带灰色十字虚线（对齐 X 轴 / Y 轴的参考线），保留数据点与 tooltip
- 延迟 / 丢包：线路与显示名完全跟随后台 Ping 节点设置（`custom_*_name` / `node_*_name`，最多 8 条），
  顶部为各线路平均延迟 / 平均丢包徽章；未配置或未上报的线路不显示
- 磁盘 IO：读取 / 写入折线 + 当前速率、IOPS、await、利用率（探针不上报时不展示图表，只提示"未上报"）
- GPU：逐个列出型号与占用
- 所有详情卡片走同一网格，卡片间距统一 15px
...
- 在线判定阈值读面板设置 `online_threshold_seconds`（默认 300 秒，不写死）
- 面板设置 `frontend_ws_timeout_minutes > 0` 时，单次实时连接到点自动断开并提示用户续订（不静默重连）
